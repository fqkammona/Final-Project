...
exports.matchMetadataWithEvents = functions.firestore
  ...
  .onCreate(async (snapshot, context) => {
    ...
    const endTimeWindow = new Date(metadataTimestamp.getTime() + 9 * 60000); // 9 minutes after the metadata timestamp

    // Convert times to Firestore timestamps
    const metadataTimestampFirestore = admin.firestore.Timestamp.fromDate(metadataTimestamp);
    const endTimeWindowFirestore = admin.firestore.Timestamp.fromDate(endTimeWindow);
    ...
    for (const userDoc of usersSnapshot.docs) {
      ...
      eventsSnapshot.forEach(eventDoc => {
        ...
        if (arraysIntersect(event.recognizedObjects, metadata.objects) &&
          ...
          console.log(`Yes, match found for User ID: ${userDoc.id} with Event ID: ${eventDoc.id}`);
          matchFound = true;
    ...
    if (!matchFound) {
      console.log('No match found within the time window for any user.');
    }
  });
