...
exports.scheduleListeningTask = functions.firestore
    ...
    .onCreate(async (snapshot, context) => {
        ...
        const startTime = eventData.timestamp.toDate(); // The Firestore Timestamp for the event start

        const project = 'girl-c0ded';
        const queue = 'trigger-listening-function';
        const location = 'us-central1'; // Queue location must match the function location
        const url = `https://${location}-${project}.cloudfunctions.net/startListeningFunction`;
        console.log("Scheduled task URL:", url);


        const payload = { userId: context.params.userId, eventId: context.params.eventId };
